...
class Vehicle(object):
    ...
    def choose_next_state(self, predictions):
        """
        Implement the transition_function code from the
        Behavior Planning Pseudocode  classroom concept.

        INPUTS: A predictions dictionary.
            This is a dictionary of vehicle id keys with predicted
            vehicle trajectories as values.
            Trajectories are a list of Vehicle objects representing
            the vehicle at the current timestep and one timestep in the future.
        OUTPUT: The the best (lowest cost) trajectory corresponding
            to the next ego vehicle state.
        """
    ...
    def successor_states(self):
        """
        Provides the possible next states given the current state for the
        FSM discussed in the course, with the exception that lane changes
        happen instantaneously, so LCL and LCR can only transition back to KL.
        """
        if self.state == "KL":
            states = ["KL", "PLCL", "PLCR"]
    ...
    def get_kinematics(self, predictions, lane):
        ...
        if vehicle_ahead:
            ...
            if vehicle_behind:
                ...
                # Equation: front buffer <= (vcl.s - ego.s) +\
                # (vcl.v - ego.v)*t + 0.5 * (vcl.a - ego.a)*t*t
                max_velocity_in_front =\
                    (vehicle_ahead.s - self.s - self.preferred_buffer) +\
                    vehicle_ahead.v - 0.5 * self.a
                new_velocity = min(max_velocity_in_front,
    ...
    def constant_speed_trajectory(self):
        trajectory = [Vehicle(self.lane, self.s, self.v, self.a, self.state),
                      Vehicle(self.lane, self.position_at(1),
                              self.v, 0, self.state)]
        return trajectory
    ...
    def realize_next_state(self, trajectory):
        """
        Sets state and kinematics for ego vehicle using the last state of
        the trajectory.
        """
